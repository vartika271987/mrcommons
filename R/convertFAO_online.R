...
convertFAO_online <- function(x,subtype) {
  ...
  if(any(getCells(x)=="XET") & any(getCells(x)=="ETH") & !any(getCells(x)=="ERI")) {
    xERI <- x["ETH",,]
    xERI[,,] <- 0
    getCells(xERI) <- "ERI"
  ...
  if(any(getCells(x)=="XSD") & any(getCells(x)=="SDN") & !any(getCells(x)=="SSD")) {
    xSSD <- x["SDN",,]
    xSSD[,,] <- 0
    getCells(xSSD) <- "SSD"
  ...
  if (all(c("XSD", "SSD", "SDN") %in% getCells(x))){
    additional_mapping <- append(additional_mapping, list(c("XSD","SSD","y2011"), c("XSD", "SDN","y2011")))
  } else if ("XSD" %in% getCells(x) & !any(c("SSD", "SDN") %in% getCells(x)) ) {
    getCells(x)[getCells(x) == "XSD"] <- "SDN"
  } 

  ## if XCN exists, replace CHN with XCN.
  if ("XCN" %in% getCells(x)) {
    if ("CHN" %in% getCells(x)) x <- x["CHN",,,invert=TRUE]
  ...
  if(subtype %in% c("EmisAgRiceCult","Fertilizer", "FertilizerNutrients", "EmisAgCultOrgSoil","EmisLuCrop","EmisLuGrass","EmisAgSynthFerti")) {
    ...
    missing <- former[!former %in% getCells(x)]
  ...
  # Producer Prices Annual
    ...
    if(all(c("SCG","SRB") %in% getCells(x)) & !"MNE" %in% getCells(x)){
      mne <- x["SRB",,]
      dimnames(mne)[[1]] <- "MNE"
      x <- mbind(x, mne)
    }
    ...
    for(item in mapping$FAO_carcass){
      ...
      countries <- getCells(which(!is.na(x[,,item]),arr.ind=TRUE))
      countries <- setdiff(getCells(x), countries)
